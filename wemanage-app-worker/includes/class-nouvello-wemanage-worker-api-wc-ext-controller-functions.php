...
if (!class_exists('Nouvello_Wemanage_Worker_Api_WC_Ext_Controller_functions')) :
    ...
    final class Nouvello_Wemanage_Worker_Api_WC_Ext_Controller_Functions
    {
        ...
        public function __construct()
        {
            ...
            // sales report.
            add_filter('woocommerce_rest_prepare_report_sales', array($this, 'custom_change_report_sales_response_v3'), 20, 3);
            // top sellers.
        ...
        public function nouvello_wp_get_categories_hierarchy_html($data, $args = '')
        {
            ...
            if ('' != $data) { // we have $data sent via GET or POST.
                $params = $this->get_passed_api_params($data);
            } elseif ('' == $data && '' != $args) { // we have $args instead of passed $data from a GET or a POST.
                $params = $args;
            }
        ...
        public function custom_change_order_response($response, $object, $request)
        {
            ...
            if (!empty($response->data) && !empty($response->data['meta_data'])) {
                foreach ($response->data['meta_data'] as $key => $meta_item) {
                    if (isset($meta_item->key) && $meta_item->key == '_wc_order_attribution_utm_source') {
                        if (isset($meta_item->value)) {
                            $response->data['utm_source'] = $meta_item->value;
                        }
                    }
                }
            }
            ...
            $response->data['currency'] = get_woocommerce_currency_symbol();
        ...
        public function nouvello_get_this_product_variation_attributes_and_terms($variations_array)
        {
            ...
            foreach ($variations_array as $variation) {
                ...
                foreach ($variation['attributes'] as $attribute) {
                    ...
                    } elseif ('custom' == $attribute['attr_type']) {
        ...
        public function get_products_price_range()
        {
            ...
            // $prices = $wpdb->get_row($prepared_sql);
			...
			FROM {$wpdb->posts} 
			LEFT JOIN {$wpdb->postmeta} as price_meta ON {$wpdb->posts}.ID = price_meta.post_id %s %s
			WHERE {$wpdb->posts}.post_type IN ('product')
				AND {$wpdb->posts}.post_status = 'publish'
				AND price_meta.meta_key IN ('_price')
				AND price_meta.meta_value > '' %s %s", $tax_query_sql['join'], $meta_query_sql['join'], $tax_query_sql['where'], $meta_query_sql['where']));
        ...
        public function nouvello_upload_csv_file($data)
        {
            ...
            if (!$filesystem) {
                // Handle error: unable to initialize the filesystem
                return new WP_Error('filesystem_error', __('Unable to initialize the filesystem.', 'nouvello-wp-management'), array('status' => 500));
            }
            ...
            $ext = pathinfo($csv_nm, PATHINFO_EXTENSION);
            $fn_name = time() . '.' . $ext;
            $filepath_nm = trailingslashit($upload_dir) . $fn_name;

            // Open the file for writing
            if (!$filesystem->put_contents($filepath_nm, '')) {
                // Handle error: unable to open the file
                return new WP_Error('file_open_error', __('Unable to open the file for writing.', 'nouvello-wp-management'), array('status' => 500));
            }

            // Write the CSV label
            $csv_label_str = implode(',', $csv_label) . "\n";
            if (!$filesystem->append_to_file($filepath_nm, $csv_label_str)) {
                // Handle error: unable to write CSV label
                return new WP_Error('csv_label_write_error', __('Unable to write CSV label to the file.', 'nouvello-wp-management'), array('status' => 500));
            }

            // Write each row of CSV data
            foreach ($csv_data as $csv_inner) {
                $csv_inner_str = implode(',', $csv_inner) . "\n";
                if (!$filesystem->append_to_file($filepath_nm, $csv_inner_str)) {
                    // Handle error: unable to write CSV data
                    return new WP_Error('csv_data_write_error', __('Unable to write CSV data to the file.', 'nouvello-wp-management'), array('status' => 500));
                }
            }

            // Validate if the uploaded file is a CSV file
            $valid_csv = false;
            $file_extension = strtolower(pathinfo($fn_name, PATHINFO_EXTENSION));
            ...
            if (!$valid_csv) {
                // Handle error: uploaded file is not a CSV file
                // Delete the invalid file
                $filesystem->delete($filepath_nm);
                return new WP_Error('invalid_file_type', __('Uploaded file is not a CSV file.', 'nouvello-wp-management'), array('status' => 400));
                // Return an error response or throw an exception
            } else {
                // Continue processing since the file is a valid CSV file
            }
         ...
         * @return [type]             [description]
         */
        public function custom_change_report_sales_response_v2($response, $object, $request)
        {
            ...
            if (!empty($response->data['totals'])) {
                foreach ($response->data['totals'] as $key => $value) {
                    ...
                    if ('week' == $period) { // not used - we use date range instead.
                        $response->data['totals'][$key]['clean_key'] = $date->format('D');
                    } elseif ('month' == $period || 'last_month' == $period) { // not used - we use date range instead.
                        $response->data['totals'][$key]['clean_key'] = intval($date->format('d'));
                    } elseif ('year' == $period) { // used intead of date range.
                        $response->data['totals'][$key]['clean_key'] = $date->format('M');
        ...
        * @return [type]             [description]
        */
        public function custom_change_report_sales_response_v3($response, $object, $request)
        {
            ...
            if (!empty($response->data['totals'])) {
                foreach ($response->data['totals'] as $key => $value) {
                    ...
                    if ('week' == $period) { // not used - we use date range instead.
                        ...
                        $datetime = new DateTime();
                        $date_end = $datetime->format('Y-m-d'); // now.

                        global $wpdb;
                        $table_name = $wpdb->prefix . 'nouvello_visitor_counter';
                        // @codingStandardsIgnoreStart
                        $total_views = $wpdb->get_results($wpdb->prepare('SELECT SUM(`views`) AS "views" FROM `' . $table_name . '` WHERE `date` BETWEEN %s AND %s', $date_start, $date_end));
                        // @codingStandardsIgnoreEnd
                        if (isset($total_views) && isset($total_views[0]) && isset($total_views[0]->views)) {
                            $response->data['total_views'] = $total_views[0]->views;
                        } else {
                        ...
                        $date_start = $request['date_min'];
                        $date_end = $request['date_max'];
                        ...
                        // another method we can use to get get all views and unique visitors of all posts in the date range
                        // $nouvello_counter_data = nouvello_wemanage_worker()->visitor_counter->nouvello_get_counter_by_date( $request['date_min'], $request['date_max'] );
                        // $requested_date_range_all_unique_visitors = 0;
                        // $requested_date_range_all_views = 0;
                        // foreach ($nouvello_counter_data as $post_data) {
                        // 	$requested_date_range_all_unique_visitors += $post_data->visitors;
                        // 	$requested_date_range_all_views += $post_data->views;
                        // }

                        // debugging
                        // return array(
                        // 	'data' => $nouvello_counter_data,
                        // 	'requested_date_range_all_unique_visitors' => $requested_date_range_all_unique_visitors,
                        // 	'requested_date_range_all_views' => $requested_date_range_all_views,
                        // );
        ...
        /**
         * Get the summary of orders with utm source grouped by time periods and by sources.
         * @param  [type] $data [description].
         * @return [type]       [description]
         */
        public function nouvello_get_utm_orders_summary($data){
            $time_periods = array('today', 'week', 'month', 'year');
            $stats = array();
            foreach ($time_periods as $period ) {
                $stats[$period] = $this->get_utm_stats($period);
            }
            return $stats;
        }
         ...
         * @return [type]       [description]
         */
        public function get_utm_stats($period){
            ...
            switch($period) {
                case 'today':
                    // today
                    $start_date = gmdate('Y-m-d 00:00:00', strtotime($end_date));
                    break;
                case 'week':
                    // this week
                    $start_date = gmdate('Y-m-d H:i:s', strtotime('last Sunday', strtotime($end_date)));
                    // last week (one week backwards from today)
                    // $start_date = gmdate('Y-m-d H:i:s', strtotime('-1 week', strtotime($end_date)));
                    break;
                case 'month':
                    // this month
                    $start_date = gmdate('Y-m-01 00:00:00', strtotime($end_date));
                    break;
                case 'year':
                    // this year
                    $start_date = gmdate('Y-01-01 00:00:00', strtotime($end_date));
                    break;
            }
            ...
            $results = $wpdb->get_results($wpdb->prepare("
                SELECT 
                    ...
                    MAX(CASE WHEN pm.meta_key = '_wc_order_attribution_utm_source' THEN pm.meta_value END) as origin
                FROM 
                    {$wpdb->prefix}posts p
                JOIN 
                ...
                ORDER BY 
                    p.post_date DESC;
            ", 'wc-completed', 'wc-processing', $start_date, $end_date));

            $data = array();

            if ($results) {
                // Initialize arrays to store total sales and count of orders by origin
                $sales_by_origin = array();
                $count_by_origin = array();

                // Loop through each order and accumulate totals and counts by origin
                foreach ($results as $row) {
                    $origin = $row->origin;
                    $total_sales = floatval($row->order_total);

                    // Count number of orders by origin
                    if (isset($count_by_origin[$origin])) {
                        $count_by_origin[$origin]++;
                    } else {
                        $count_by_origin[$origin] = 1;
                    }

                    // Sum total sales by origin
                    if (isset($sales_by_origin[$origin])) {
                        $sales_by_origin[$origin] += $total_sales;
                    } else {
                        $sales_by_origin[$origin] = $total_sales;
                    }
                }

                // Prepare the final array data
                foreach ($sales_by_origin as $origin => $total_sales) {
                    $data[] = array(
                        'origin' => $origin,
                        'order_count' => $count_by_origin[$origin],
                        'total' => $total_sales,
                        'currency' => get_woocommerce_currency_symbol(),
                    );
                }
        ...
        /**
         * Get the info for orders with utm source.
         *
         * @param  [type] $data [description].
         * @return [type]       [description]
         */
        public function nouvello_get_utm_orders_info($data)
        {
            ...
            // this week
            $start_date = gmdate('Y-m-d H:i:s', strtotime('last Monday', strtotime($end_date)));
