...
class Nouvello_WeManage_Worker_Visitor_Counter
{
    ...
    {
        wp_register_script('nouvello-visitor-counter', NSWMW_ROOT_DIR . '/includes/assets/js/visitor-counter.min.js', array( 'jquery' ), 1, 1);
        wp_localize_script(
    ...
    public function nouvello_update_counter_cb()
    {
        ...
        if (! isset($_COOKIE['nouvello_unique_visitor'])) {
            ...
            setcookie('nouvello_unique_visitor', '1', time() + (60 * 30), '/', parse_url(site_url(), PHP_URL_HOST)); // 30 minutes
            $visitors = 1;
            $views = 1;
        }
     ...
     * @return [type]           [description]
     */
    public function nouvello_get_counter_by_date($date_min = '', $date_max = '')
    {
        global $wpdb;
        $table_name = $wpdb->prefix . 'nouvello_visitor_counter';
        // @codingStandardsIgnoreStart
        $post_data = $wpdb->get_results($wpdb->prepare('SELECT * FROM `' . $table_name . '` WHERE `date` BETWEEN %s AND %s', $date_min, $date_max));
        // @codingStandardsIgnoreEnd
        if (isset($post_data) && isset($post_data[0])) {
            return $post_data;
        }
