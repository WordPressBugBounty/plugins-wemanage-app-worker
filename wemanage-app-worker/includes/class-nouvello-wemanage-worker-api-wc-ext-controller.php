    ...
    final class Nouvello_Wemanage_Worker_Api_WC_Ext_Controller
    {
        ...
        public function register_routes()
        {
            register_rest_route(
                ...
                array(
                    ...
                    'permission_callback' => function ($request) {
                        return $this->verify_permissions($request);
                    },
                )
            );
            ...
            register_rest_route(
                ...
                array(
                    'methods'  => 'GET',
                    'callback' => array( nouvello_wemanage_worker()->wc_api_functions, 'nouvello_get_utm_orders_summary' ),
                    'permission_callback' => function ($request) {
                        return $this->verify_permissions($request);
                    },
                )
            ...
            register_rest_route(
                ...
                array(
                    'methods'  => 'GET',
                    'callback' => array( nouvello_wemanage_worker()->wc_api_functions, 'nouvello_get_utm_orders_info' ),
                    'permission_callback' => function ($request) {
                        return $this->verify_permissions($request);
                    },
                )
        ...
        public function verify_permissions($request)
        {
            ...
            if (!is_user_logged_in()) {
                return false;
                // return new WP_Error('rest_forbidden', __('You are not logged in.', 'text-domain'), array( 'status' => 401 ));
            }

            // Check if the logged-in user is an administrator
            if (!current_user_can('administrator')) {
                return false;
                // return new WP_Error('rest_forbidden', __('You are not authorized to access this resource.', 'text-domain'), array( 'status' => 403 ));
            }

            return true; // User is logged in and is an administrator
        }
